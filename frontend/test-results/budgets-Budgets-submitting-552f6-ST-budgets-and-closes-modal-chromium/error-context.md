...
- Name: budgets.spec.ts >> Budgets >> submitting valid budget form calls POST /budgets and closes modal
- Location: e2e\budgets.spec.ts:193:3
...
Call log:
  - waiting for getByLabel('Category')
    - locator resolved to <select id="budget-category" class="flex h-10 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:border-primary">…</select>
