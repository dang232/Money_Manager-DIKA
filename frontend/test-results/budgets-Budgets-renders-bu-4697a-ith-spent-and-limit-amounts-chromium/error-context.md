...
- Name: budgets.spec.ts >> Budgets >> renders budget cards with spent and limit amounts
- Location: e2e\budgets.spec.ts:106:3
...
Locator: locator('.h-2.bg-muted.rounded-full').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.h-2.bg-muted.rounded-full').first()
